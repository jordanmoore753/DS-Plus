...
describe('Graph', () => {
  ...
  it('should be initialized with a string', () => {
    expect(() => {
      new Plus.Graph();
    }).toThrow('Data type must be: string, number, object.');

    expect(() => {
      new Plus.Graph('random');
    }).toThrow('Data type must be: string, number, object.');

    let g = new Plus.Graph('string');
    expect(g).toEqual({
      data: {},
      type: 'string',
      size: 0
  ...
  it('should add keys as values to data', () => {
    ...
    expect(() => {
      g.addVertices(1);
    }).toThrow('Vertice already exists in graph.');
    ...
    g.addVertices({ id: 1 });
    expect(g.data[{ id: 1 }.toString()]).toEqual([]);
    ...
    expect(g.data[{ id: 2 }.toString()]).toEqual([]);
    expect(g.data[{ id: 3 }.toString()]).toEqual([]); 

    // string

    g = new Plus.Graph('string');
  ...
  it('should link keys to specific values in array', () => {
    ...
    expect(() => {
      g.addEdges(2, [1]);
    }).toThrow('Value in edges array not a valid vertex.');

    expect(() => {
      g.addEdges(2, 3, 4, 5);
    }).toThrow('Edges must be in array.');

    expect(() => {
      g.addEdges(2, [2]);
    }).toThrow('Value in edges array cannot be the first argument.');
    ...
    expect(() => {
      g.addEdges(2, [3]);
    }).toThrow('Vertex already exists as connection for passed vertex.');
    ...
    expect(g.data[{ id : 1 }]).toEqual([{ id: 2 }, { id: 3 }, { id: 4 }]);
